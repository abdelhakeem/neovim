...
/// @param  flags  Flags, see ContextTypeFlags enum.
///
/// @return Error indicating context restore result.
    ...
    while (!need_more && unpacker->off < unpacker->used) {
      ...
      switch (ret) {
        ...
        case MSGPACK_UNPACK_EXTRA_BYTES:
          semsg(_(e_intern2), "Context: extra bytes in msgpack string");
          goto exit;
        case MSGPACK_UNPACK_PARSE_ERROR:
          semsg(_(e_intern2), "Context: failed to parse msgpack string");
          goto exit;
